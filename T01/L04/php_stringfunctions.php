...
<p>
  <?php echo str_repeat('  **REPEAT THIS** ', 10 .'<br>');?>
</p>

<h2>strrchr($string, $char) : find the last occurence of the character $char in $string</h2>
<p>For example: you want to get the file extension from a file name. </p>
<p>
<?php
...
<p>
  <?php  echo trim('  abc def      ');?> <!--useful for forms -> database-->
</p>
...
<?php
  $name="O'Brien"; //this string contains a special character !! cuases problems when brought into SQL
...
<h2>explode($separator, $string) : Split $string by $separator</h2>
  <p>This function is commonly used to extract values in a string which are separated by a a certain separator
...
<?php
  ...
  $info = explode(',', $csv);  //this creates an array $info. comma is the seperator
?>

<p> Now, $info is an array with three values : </p>
...
<p>We can further process this array like displaying them in a table.</p>
<p>
<?php
  echo "<table border='1'>";
  ...
  } //foreach loop goes to each iteration in the array and then exits
  echo '</table>';
?>
</p>
...
<p>
Another example : If we have an array containing some values and we want to print them in an ordered
